...
import br.edu.iff.ccc.bsi.forumhub.model.Postagem;
...
public class PostagemController {
	...
	public ResponseEntity<Postagem> getPostagemById(@PathParam("id") int id) {
		if (id < 0) {
	...
	public ResponseEntity<Void> postPostagem(@RequestBody Postagem postagem) {
		if (postagem == null) {
			return ResponseEntity.badRequest().build();
		}
		Postagem novaPostagem = postagem;
	...
	public ResponseEntity<Void> updatePostagem(@RequestBody Postagem postagem) {
		if (postagem == null) {
			return ResponseEntity.badRequest().build();
		}
		Postagem novaPostagem = postagem;
